...
class Settings extends Access
{
	...
	function add()
	{
		...
		if ($this->input->server('REQUEST_METHOD')=='POST') {			
			...
			if (!isset($upload_data['error'])) {
				...
				if ($this->setting->save($feed_data)) {
		...
		$content['content'] = $this->load->view('settings/add',array(),true);
	...
	function index()
	{
		...
		$pag['base_url'] = site_url('settings/index');
		$pag['total_rows'] = $this->setting->count_all();
		
		$data['aboutsdata'] = $this->setting->get_all($pag['per_page'],$this->uri->segment(3));
		$data['pag'] = $pag;
		
		$content['content'] = $this->load->view('settings/view',$data,true);		
	...
	function search()
	{
		...
		$pag['base_url'] = site_url('settings/search');
		$pag['total_rows'] = $this->setting->count_all_by($search_term);
		
		$data['aboutsdata'] = $this->setting->get_all_by($search_term,$pag['per_page'],$this->uri->segment(3));
		$data['pag'] = $pag;
		
		$content['content'] = $this->load->view('settings/search',$data,true);		
		$this->load->view('template',$content);
	}
	...
	function edit($feed_id=0)
	{
		...
		if ($this->input->server('REQUEST_METHOD')=='POST') {
			if ($this->setting->save($this->input->post(),$feed_id)) {
		...
		$data['news'] = $this->setting->get_info($feed_id);
		
		$content['content'] = $this->load->view('settings/edit',$data,true);		
		$this->load->view('template',$content);
	}
	...
	function publish($id = 0)
	{
		...
		if ($this->setting->save($feed_data,$id)) {
			echo 'true';
		} else {
			echo 'false';
		}
	...
	function delete($feed_id=0)
	{
		...
		if ($this->setting->delete($feed_id)) {
	...
	function delete_image($feed_id,$image_id,$image_name)
	{
		...
		redirect(site_url('settings/edit/'.$feed_id));
	}
}
?>