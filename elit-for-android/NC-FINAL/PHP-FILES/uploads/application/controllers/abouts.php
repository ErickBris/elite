...
class Abouts extends Access
{
	...
	function index()
	{
		...
		$pag['base_url'] = site_url('abouts/index');
		$pag['total_rows'] = $this->about->count_all();
		
		$data['aboutsdata'] = $this->about->get_all($pag['per_page'],$this->uri->segment(3));
		$data['pag'] = $pag;
		
		$content['content'] = $this->load->view('abouts/view',$data,true);		
	...
	function search()
	{
		...
		$pag['base_url'] = site_url('abouts/search');
		$pag['total_rows'] = $this->about->count_all_by($search_term);
		
		$data['aboutsdata'] = $this->about->get_all_by($search_term,$pag['per_page'],$this->uri->segment(3));
		$data['pag'] = $pag;
		
		$content['content'] = $this->load->view('abouts/search',$data,true);		
		$this->load->view('template',$content);
	}
	...
	function edit($feed_id=0)
	{
		...
		if ($this->input->server('REQUEST_METHOD')=='POST') {
			if ($this->about->save($this->input->post(),$feed_id)) {
		...
		$data['news'] = $this->about->get_info($feed_id);
		
		$content['content'] = $this->load->view('abouts/edit',$data,true);		
		$this->load->view('template',$content);
	}
	...
	function publish($id = 0)
	{
		...
		if ($this->about->save($feed_data,$id)) {
			echo 'true';
		} else {
			echo 'false';
		}
	...
	function delete_image($feed_id,$image_id,$image_name)
	{
		...
		redirect(site_url('abouts/edit/'.$feed_id));
	}
}
?>