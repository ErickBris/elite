...
class Itemsmix extends REST_Controller
{
	function get_get()
	{
	...
	$data  = array();
		...
		$item = $this->itemmix->get_info($item_id);
	...
	function allmix_get()
	{
		...
		$allthemix = $this->mix->get_all()->result();
		$dataz = array();
		$data = array();
		//$j = 0;
		foreach ($allthemix as $itemmix) {
			//$item->images = $this->image->get_all_by_item($item_id)->result();
			$data[] =$itemmix;
			///$dataz = array($data);
